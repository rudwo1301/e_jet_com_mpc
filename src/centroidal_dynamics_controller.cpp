...
rp_clik.topRows<3>() = rfoot_trajectory_float_.translation() - rfoot_float_current_.translation();
 ...
 void WalkingController::QPController(VectorQd& optimal_q_dot)
 {
    ...
    qpOASES::real_t H_input[15*15], A_input[12*15], lbA_input[12], ubA_input[12], lb_input[15],ub_input[15], g_input[15];
    ...
    for(int j=0;j<15;j++)
    {
     ...
     qpOASES::real_t qOpt[15];
     qpOASES::QProblem test(15,12);

     qpOASES::Options options1;
     options1.initialStatusBounds = qpOASES::ST_INACTIVE;
     options1.numRefinementSteps = 1;
     options1.enableCholeskyRefactorisation = 1;
     options1.printLevel = qpOASES::PL_NONE;

     test.setOptions(options1);

     qpOASES::int_t nWSR1= 1000;
